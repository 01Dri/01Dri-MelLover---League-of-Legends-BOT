...
class ApiRioIntegrationTests(unittest.TestCase):
    ...
    def test_get_tier_account_flex(self):
        riot_api = ApiRiot("a bela","bela", TOKEN_RIOT, "RANKED_FLEX_SR")
        result = riot_api.get_all_info_account_league()
        self.assertEqual("EMERALD", result["tier"])

    def test_get_rank_account_flex(self):
        riot_api = ApiRiot("a bela","bela", TOKEN_RIOT, "RANKED_FLEX_SR")
        result = riot_api.get_all_info_account_league()
        self.assertEqual("I", result["rank"])

    def test_get_league_points_account_flex(self):
        riot_api = ApiRiot("a bela","bela", TOKEN_RIOT, "RANKED_FLEX_SR")
        result = riot_api.get_all_info_account_league()
        self.assertEqual(76, result["lp"])

    def test_get_winrate_account_flex(self):
        riot_api = ApiRiot("a bela","bela", TOKEN_RIOT, "RANKED_FLEX_SR")
        result = riot_api.get_all_info_account_league()
        self.assertEqual(60, result["winrate"])
    ...
    def test_get_check_account_without_info_queue_type(
            self):  # This test verify if account is UNRANKED in specify queue
